...
class UpstreamQuotaError(UpstreamError):
    ...
    实测（2026-09-24 归因）：它其实是**按出口 IP** 的软限 —— 同 IP 换身份行为同型，
    换新 IP 立刻恢复 ⇒ **不给静默期**，但**值得换出口重试**：服务侧默认
    **自动重试一次**（`TEXTIN_SOFT_LIMIT_RETRY` 可调，0=关；每次重试都新建连接换出口）。
    """
...
_CODE_GUIDANCE: dict[int, str] = {
    451: (
        ...
        "（实测同一出口连打：成功×3 → 失败 → 成功×2；换出口立刻恢复），"
        "本服务已**自动换出口重试一次**（`TEXTIN_SOFT_LIMIT_RETRY=0` 可关）——"
        "仍失败时再考虑配 TEXTIN_TOKEN 或登录"
    ),
    431: (
